...
def upload_to_mongodb(articles, collection):
    ...
    for article in articles:
        ...
        if collection.find_one(query):
            print(f"  ⏭️ [跳過] 已存在: {article['title'][:15]}...")
            continue
...
def test_job():
    print("=== 🟢 啟動終極模組化 ETL 測試 ===")
    
    # 【重點 2】：收集資料變得超級簡單！
    print("\n[階段一：呼叫爬蟲模組提取資料]")
    all_articles = []
    all_articles.extend(get_api_articles(test_mode=True))  # 從 API 拿 6 篇
    all_articles.extend(get_tfc_articles(test_mode=True))  # 從 TFC 拿 3 篇
    
    print(f"\n🏁 階段一完成！總共收集到 {len(all_articles)} 篇「格式已統一」的文章。")
    
    print("\n[階段二：切片與上傳]")
    try:
        client = MongoClient(MONGO_URI)
        collection = client["CARE_database"]["test_health_articles_chunks"]
        total_new = upload_to_mongodb(all_articles, collection)
        print(f"\n=== 🔴 測試結束！成功上傳了 {total_new} 篇全新文章 ===")
    except Exception as e:
